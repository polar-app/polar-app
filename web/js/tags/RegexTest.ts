...
function assertCaptureCount(expectedCount: number, regexp: RegExp, sample: string) {

    assertTrue(regexp.test(sample),
               "Pattern failed to match sample: '" + sample + "'");

    const groups = regexp.exec(sample);

    assert.ok(groups, `No matches found and expected count ${expectedCount}`);

    assert.equal(groups!.length,
                 expectedCount,
                 "Does not have " + expectedCount + " captures as expected: '" + sample + "'");

}

describe('AbstractMethodCalls', function() {

    it("testAutoLinkHashtags", function() {
