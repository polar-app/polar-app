import {MediaContents} from './MediaContents';
import {assertJSON} from '../../../test/Assertions';

describe('MediaContents', function() {

    describe('parseDataURLs', function() {

        it("basic", async function () {

            let content = "What is the capital of California? <img src=\"data:image/gif;base64,\">" ;

            let expected = [
                "data:image/gif;base64,"
            ];

            assertJSON(MediaContents.parseDataURLs(content), expected);

        });

    });

    describe('toMediaFile', function() {

        it("basic", async function () {

            let dataURL = "data:image/gif;base64,"

            let expected: any = {
                    "filename": "1hqKtrsGiLerE69bmb5i.gif",
                    "data": ""
                }
            ;

            assertJSON(MediaContents.toMediaFile(dataURL).get(), expected)

        });

    });


});
