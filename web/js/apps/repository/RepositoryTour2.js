...
exports.RepositoryTour2 = exports.Styles = void 0;
...
exports.RepositoryTour2 = ReactUtils_1.deepMemo(() => {
    const [state, setState] = React.useState({
        run: !LifecycleToggle_1.LifecycleToggle.isMarked(LifecycleEvents_1.LifecycleEvents.TOUR_TERMINATED),
        stepIndex: 0
    });
    const steps = useSteps();
    const doStep = React.useCallback((callBackProps) => {
        const stepIndex = callBackProps.index + (callBackProps.action === react_joyride_1.ACTIONS.PREV ? -1 : 1);
        setState(Object.assign(Object.assign({}, state), { stepIndex }));
    }, [state]);
    const handleCallback = React.useCallback((callbackProps) => {
        const step = callbackProps.step;
        if (callbackProps.action === 'update' && step.autoNext) {
            const nextStep = steps[callbackProps.index + 1];
            const nextHandler = () => {
            ...
            const mutationHandler = () => {
                if (nextHandler()) {
                    ...
                    setState(Object.assign(Object.assign({}, state), { stepIndex, run: false }));
                }
            };
        ...
        if (callbackProps.status === react_joyride_1.STATUS.SKIPPED || callbackProps.status === react_joyride_1.STATUS.FINISHED) {
            ...
            if (!state.run) {
                setTimeout(() => {
                    setState(Object.assign(Object.assign({}, state), { run: true }));
                }, 250);
                return;
            }
    ...
    return (React.createElement(react_joyride_1.default, { steps: steps, continuous: true, callback: data => handleCallback(data), run: state.run, showProgress: true, showSkipButton: true, stepIndex: state.stepIndex, styles: {
            options: {
                primaryColor: '#007bff',
                zIndex: Z_INDEX,
            },
...
//# sourceMappingURL=RepositoryTour2.js.map