...
export class Dates {
    ...
    public static addDays(dateA: DateLike, numDays: number) {
        const date = new Date(dateA);
        date.setTime(date.getTime() + daysToMS(numDays));
        return date;
    }

    public static subtractDays(dateA: DateLike, numDays: number) {
        const date = new Date(dateA);
        date.setTime(date.getTime() - daysToMS(numDays));
        return date;
    }

    public static diffDays = (dateA: Date, dateB: Date) => {
