...
async function doRegression(opts: ExecutorOpts) {
    ...
    engine.xregister("covid 1", async function () {

        const answer = await executor.executeQuestion("What happened after a single dose of BNT162b2 vaccine?");
    ...
    engine.xregister("covid 2", async function () {
        // TODO: should work

        const answer = await executor.executeQuestion("What do two doses of SARS-CoV-2 vaccination induce?");

        assert.equal(answer, "")

    });

    engine.xregister("covid 3", async function () {

        const answer = await executor.executeQuestion("What neutralized the prototype B virus?");
    ...
    engine.xregister("bigtable 1", async function () {

        const answer = await executor.executeQuestion("Is Bigtable relational?");

        assert.equal(answer, "No.")

    });

    engine.xregister("bigtable single-row transactions", async function () {

        const answer = await executor.executeQuestion("Does Bigtable support single-row transactions?");
    ...
    engine.xregister("bigtable general transactions", async function () {

        const answer = await executor.executeQuestion("Does Bigtable support general transactions?");

        assert.equal(answer, "No.")

    });

    engine.xregister("TODO: bigtable type of transactions", async function () {

        // TODO: not supported yet. I need to extend examples and examples_context here

        const answer = await executor.executeQuestion("What types of transactions does bigtable support?");
    ...
    engine.register("astronomy Chapter 2 #3", async function() {
        await executor.assertQuestionAndAnswer("The Sun was once thought to be a planet. Explain why.", [
    ...
    engine.register("astronomy Chapter 2 #4", async function() {
        await executor.assertQuestionAndAnswer("How far is the nearest galaxy from the sun?", [
            "75,000 light-years from the Sun",
            "75,000 light-years",
            "75,000 light-years from the Sun in the direction of the constellation Sagittarius"
        ]);
    })
    engine.register("astronomy Chapter 2 #5", async function() {
        await executor.assertQuestionAndAnswer("What is an asterism?", [
    ...
    engine.register("astronomy Chapter 2 #6", async function() {
        await executor.assertQuestionAndAnswer("Give at least one of Aristotle's arguments why he considered the earth to be round", [
    ...
    engine.register("astronomy Chapter 2 #7", async function() {
        // TODO: we don't have a proper text to discern this...

        await executor.assertQuestionAndAnswer("How are the zodiacal constellations different from the other constellations?", [
    ...
    // Chapter 3
    engine.register("astronomy Chapter 3 #1", async function() {
        await executor.assertQuestionAndAnswer("What is the orbital speed?", [
    ...
    engine.register("astronomy Chapter 3 #2", async function() {
        await executor.assertQuestionAndAnswer("What's Newton's first law?", [
    ...
    engine.register("astronomy Chapter 3 #3", async function() {
        await executor.assertQuestionAndAnswer("Which major planet has the largest semimajor axis?", [
            "Jupiter",
            "Jupiter, The Solar System's Largest Planet"
        ]);
    })
    engine.register("astronomy Chapter 3 #4", async function() {
        await executor.assertQuestionAndAnswer("Which major planet has the largest average orbital speed around the Sun?", [
            "Mercury"
        ]);
    })
    engine.register("astronomy Chapter 3 #6", async function() {
        await executor.assertQuestionAndAnswer("Which major planet has the largest eccentricity?", [
            "Mercury"
        ]);
    })
...
function createExecutor(opts: ExecutorOpts) : IExecutor {
    ...
    return {executeQuestion, assertQuestionAndAnswer};
