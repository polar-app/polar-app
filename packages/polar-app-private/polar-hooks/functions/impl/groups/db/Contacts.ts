...
// import {FirestoreTypedArray} from "polar-firebase/src/";
import { FirestoreTypedArray } from "polar-firestore-like/src/Collections.ts";
...
export class Contacts {
  ...
  public static async createOrUpdate(
    ...
    const getExisting = async () => {
      ...
      const functions = [
        () =>
          contactInit.email
            ? this.getViaEmail(contactInit.email, idUser)
            : undefined,
        () =>
          contactInit.profileID
            ? this.getViaProfileID(contactInit.profileID, idUser)
            : undefined,
      ];
    ...
    const createContact = async (): Promise<Contact> => {
      ...
      if (existing) {
        const rel = Sets.toArray(
          Sets.mergedArrays(
            Arrays.toArray(existing.rel),
        ...
        return {
          id: existing.id,
          uid: existing.uid,
          profileID: Optional.first(
            existing.profileID,
            contactInit.profileID
          ).getOrUndefined(),
          email: Optional.first(
            existing.email,
            contactInit.email
          ).getOrUndefined(),
  ...
  public static async getViaEmail(
    email: EmailStr,
    idUser: IDUser
  ...
  public static async getViaProfileID(
  ...
  private static async firstRecord(
