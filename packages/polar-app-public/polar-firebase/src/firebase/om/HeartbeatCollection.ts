...
export namespace HeartbeatCollection {
  export interface IHeartbeatsInit {
    /**
  ...
  export interface IHeartbeat extends IHeartbeatsInit {}
  ...
  export class HeartbeatCollection {
    ...
    public static async write(uid: UserIDStr | undefined) {
      ...
      const doc = firestore.collection(this.COLLECTION).doc(heartbeat.id);

      await doc.set(Dictionaries.onlyDefinedProperties(heartbeat));
    }

    public static create(uid: UserIDStr | undefined): IHeartbeatsInit {
