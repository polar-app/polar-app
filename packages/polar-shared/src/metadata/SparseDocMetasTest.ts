...
describe('SparseDocMetas', function() {
    ...
    const enabled = DocMetaSerializer.ENABLE_SPARSE_DOC_SERIALIZE;

    beforeEach(() => {
        // DocMetaSerializer.ENABLE_SPARSE_DOC_SERIALIZE = true;
    });

    afterEach(() => {
        // DocMetaSerializer.ENABLE_SPARSE_DOC_SERIALIZE = enabled;
    });

    it("size without making sparse", function () {

        const docMeta = DocMetaSerializer.create('0x1234', 7500);

        const json = DocMetaSerializer.serialize(docMeta);
    ...
    it("size as sparse", function () {
        ...
        const inputDocMeta = DocMetaSerializer.create('0x1234', 7500)
