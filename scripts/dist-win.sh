...
build_for_arch() {
   ...
   # --config.publish.channel=latest-win-'${arch}'
    ...
    docker run --rm -ti \
       ...
       -v ~/.cache/electron-builder:/root/.cache/electron-builder \
       -v ${WINDOWS_CSC_DIR}:/root/windows-csc \
       electronuserland/builder:wine bash -c './node_modules/.bin/electron-builder --config=electron-builder.yml --config.'${target}'.artifactName=\${name}-\${version}-'${target}'-'${arch}'.\${ext} --'${arch}' --win '${target}' --publish always'
}
